...
DISPLAY_GRAPHS = [
    ("artificial_trees_ultrawide_100", "WT 1", None),
    ("artificial_trees_ultrawide_1000", "WT 2", None),
    ("artificial_trees_ultrawide_10000", "WT 3", None),
    ("artificial_trees_ultratall_10000", "DT", None),
    ("artificial_forests_40", "TF", None),
    ("snb_sf1_comment", "SNB/C", None),
    ("snb_sf1_place", "SNB/P", None),
    ("snb_sf1_tagclass", "SNB/T", None),
]
...
def make_placeholder_value(seed_key: str) -> float:
    random.seed(seed_key)
    return random.uniform(0.7, 3.0)
...
def log_query_values(query, speedups_by_db, display_entries, suffixes_by_db):
    display_modes_by_base = defaultdict(set)
    for entry in display_entries:
        display_modes_by_base[entry["base"]].add(entry["mode"])

    display_bases = set(display_modes_by_base.keys())
    drawable_bases = {
        base for base, modes in display_modes_by_base.items()
        if any(mode != "placeholder" for mode in modes)
    }
    ...
    for db in ordered_dbs:
        ...
        for base in sorted(base_map.keys()):
            ...
            for suffix in suffixes_by_db[db]:
                ...
                elif base not in drawable_bases:
                    render_mode = "display-placeholder"
...
def plot_query(
    ...
    display_entries = []
    for base, label, mode in DISPLAY_GRAPHS:
    ...
    for tick_label, is_placeholder in zip(ax.get_xticklabels(), placeholder_flags):
        if is_placeholder:
            tick_label.set_bbox(
                dict(
    ...
    apply_fixed_layout(fig, top=layout_top_for_query)
    file_name = f"speedup_compare_{sanitize_filename(query)}{output_db_suffix}.pdf"
...
def plot_placeholder(
    ...
    fig_height = fig_height_override if fig_height_override is not None else FIG_HEIGHT
    fig, ax = plt.subplots(
        figsize=(max(11, len(display_entries) * FIG_WIDTH_PER_GRAPH), fig_height)
    )
    placeholder_color = "#CFCFCF"
    ...
    if layout["left_dbs"]:
        left_centers = compute_centers(layout["left_start"], layout["left_graph_group_width"], layout["group_gaps"])
        for base_idx, entry in enumerate(display_entries):
            base_center = left_centers[base_idx]
            x_positions.append(base_center)
            x_labels.append(entry["label"])

    right_centers = compute_centers(layout["right_start"], layout["right_graph_group_width"], layout["group_gaps"])
    for base_idx, entry in enumerate(display_entries):
        base_center = right_centers[base_idx]
        x_positions.append(base_center)
        x_labels.append(entry["label"])
    ...
    if baseline_label_placement == "line":
        x_min, x_max = ax.get_xlim()
        baseline_x = x_min + 0.02 * (x_max - x_min)
        ax.text(
            baseline_x,
    ...
    file_name = f"speedup_compare_placeholder{output_db_suffix}.pdf"
    output_path = os.path.join(out_dir, file_name)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python scripts/plot_speedup_compare.py --age path/to/age_runtimes.csv --kuzu path/to/kuzu.csv --neo4j path/to/neo4j.csv\n"
            "  python scripts/plot_speedup_compare.py --age path/to/age_runtimes.csv --kuzu results/kuzu\n"
            "  python scripts/plot_speedup_compare.py --age results/age.csv --neo4j results/neo4j --legend-placement outside --legend-align right\n"
            "  python scripts/plot_speedup_compare.py --age results/age.csv --crop-query-max 1:300 --crop-query-max 2:120\n\n"
            "Behavior:\n"
            "  - One PDF is generated per AGE query: speedup_compare_<query>.pdf.\n"
            "  - Query selection is based on queries found in AGE input CSV(s).\n"
    ...
    parser.add_argument(
        "--show-baseline-label",
        action="store_true",
        help="Deprecated alias for --baseline-label-placement line.",
    )
    parser.add_argument(
        "--create-placeholder",
        action="store_true",
        help="Also create placeholder plot (default: disabled).",
    )
    parser.add_argument(
        "--label-shift",
        action="append",
    ...
    if args.show_baseline_label:
        baseline_label_placement = "line"
    ...
    for query in queries:
        ...
        max_query_fig_height = max(max_query_fig_height, fig_height)

    if args.create_placeholder:
        plot_placeholder(
            [dict(base=base, label=label, mode=mode) for base, label, mode in DISPLAY_GRAPHS],
            out_dir,
            y_limits,
            active_dbs,
            suffixes_by_db,
            output_db_suffix,
            baseline_label_placement,
            fig_height_override=max_query_fig_height,
        )
