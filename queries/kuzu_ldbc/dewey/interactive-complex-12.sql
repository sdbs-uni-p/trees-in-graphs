-- SPDX-License-Identifier: GPL-3.0-only
MATCH (base:TagClass {dewey: $baseDewey})
MATCH (:Person {id: 933})-[:KNOWS]-(friend:Person)
      <-[:HAS_CREATOR]-(comment:Comment)-[:REPLY_OF]->(:Post)
      -[:HAS_TAG]->(tag:Tag)-[:HAS_TYPE]->(tagClass:TagClass)
WHERE tag.name = 'Monarch' OR tagClass.dewey = $baseDewey
      OR tagClass.dewey STARTS WITH ($baseDewey + '.')
RETURN friend.id AS personId, friend.firstName AS personFirstName,
       friend.lastName AS personLastName, collect(DISTINCT tag.name) AS tagNames,
       count(DISTINCT comment) AS replyCount
ORDER BY replyCount DESC, personId ASC LIMIT 20;
